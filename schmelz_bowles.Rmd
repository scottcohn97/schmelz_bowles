...
```{r}
# Save plot
ggsave(file = "output/schmelz_bowles_01.pdf", plot = p3, width = 7, height = 7)
ggsave(file = "output/schmelz_bowles_01.png", plot = p3, width = 7, height = 7)
```
...
```{r}
# Save plot
ggsave(file = "output/schmelz_bowles_02.pdf", plot = g3, width = 7, height = 7)
ggsave(file = "output/schmelz_bowles_02.png", plot = g3, width = 7, height = 7)
```
...
pdf_plot1 <- df_norm %>%
  ...
  geom_line(aes(x = x1, y = y1p, color = "#c10534"), linetype = 1, size = 1.2) +
  geom_line(aes(x = x2, y = y2p, color = "#0000ff"), linetype = 1, size = 1.2) + 
  NULL
...
cdf_plot2 <- cdf_plot1 + 
  ...
  scale_color_manual(name = element_blank(),
    ...
    labels = c(bquote(italic(f[t]) * ", vaccinated at " * italic(t)),
               ...
               bquote(italic(p[t+1]^v) * italic((f[t])) * ", prefer being vacc at " * italic(t+1) * " if vol")),
               guide = guide_legend(reverse = TRUE)) +  
  labs(x = bquote(delta (f)), y = "Fraction") + 
  coord_equal() +
  theme_minimal() + 
  theme(
    panel.grid = element_blank(),
...
```{r}
ggsave("output/trunc_cdf_plot.pdf", plot = cdf_plot2, device = cairo_pdf())
ggsave("output/trunc_cdf_plot.png", plot = cdf_plot2)
```
